...
def load_llff_data(args, basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_epi=False):
    poses, bds, imgs, quaternion, velocity = _load_data(basedir, factor=factor)  # factor=8 downsamples original imgs by 8x
    print('Loaded', basedir, bds.min(), bds.max())

    # Correct rotation matrix ordering and move variable dim to axis 0
    # original transforation: from llff to NeRF coor-system
    poses = np.concatenate([poses[:, 1:2, :], -poses[:, 0:1, :], poses[:, 2:, :]], 1)
    # ###########################
    # To use our dataset, first call llff_conversion.py to \
    # generate poses_bounds.npy, and use following transformation:
    # From colmap to NeRF coor-system
