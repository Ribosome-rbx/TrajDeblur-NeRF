...
class DSKnet(nn.Module):
    def __init__(self, num_img, poses, num_pt, kernel_hwindow, *, quaternion=None, velocity=None, random_hwindow=0.25,
        ...
        NOTE: our modification -> Adding Trajectory Information
        ...
        # construct quaternion and velocity embed functions
        if quater_embed > 0:
        ...
        traj_embedding = None
        if (self.quater_embed_fn is not None) and (self.velocity_embed_fn is not None):
            ...
            # normalize quaternion
            velocity = (velocity - torch.min(velocity, dim=0)[0]) / (torch.max(velocity, dim=0)[0]-torch.min(velocity, dim=0)[0])
            velo = self.velocity_embed_fn(velocity)
            # concate quaternion and velocity -> trajectory embedding
            traj_embedding = torch.cat([quater, velo], dim=-1).type(torch.float32)

        # modify img_embed into the length of Trajectoy_Embedding
        if (traj_embedding is not None): img_embed = traj_embedding.shape[-1]
        ...
        if img_embed > 0:
            # use Trajectoy_Embedding (not learnable)
            if traj_embedding is not None: self.img_embed = traj_embedding 
            else: # if not use traj, view embedding is initized as zero (learnable)
